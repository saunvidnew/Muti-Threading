...
void* ThFindMinWithSemaphore(void *param) {
	...
	for(int i = startIndex; i < endIndex; i++){
		if(gData[i] == 0){
			printf("found 0\n");
            // Found zero, set min to 0, update thread status, post semaphore and exit
            gThreadMin[threadNum] = 0;
            gThreadDone[threadNum] = true;
            sem_post(&completed);
            return NULL;  // Exit the thread function immediately
        }
...
void CalculateIndices(int arraySize, int thrdCnt, int indices[MAX_THREADS][3]) {
    ...
    for(int i = 0; i < thrdCnt; i++){
        ...
        // Add one extra element to the first 'remainder' threads
        int extra = (i < remainder) ? 1 : 0;
        int partitionSize = lengthDivision + extra;
        
        // Calculate end index (exclusive)
        indices[i][2] = startIndex + partitionSize;
